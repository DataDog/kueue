...
- `ACRetryAfter` feature gate (disabled by default in alpha)
- rework delaying mechanism to support multiple updates to `requeueState.requeueAt`
- add new fields (`RetryAfterSeconds` and `RetryCount`) to admission check state
...
When an admission check includes this configuration and sets its state to Retry, Kueue would compute the delay and populate the `retryAfterSeconds`` field defined in this KEP.
